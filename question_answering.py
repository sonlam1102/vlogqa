...
from transformers import (
    ...
    DebertaV2Tokenizer,
    DebertaV2ForTokenClassification, AutoModel
)
...
class QuestionAnsweringModel:
    def __init__(
        self, model_type, model_name, args=None, use_cuda=True, cuda_device=-1, use_fast=False, **kwargs
    ):
        ...
        MODEL_CLASSES = {
            ...
            "mbart": (MBartConfig, MBartForQuestionAnswering, MBartTokenizer),
            "bartpho_s": (MBartConfig, MBartForQuestionAnswering, BartphoTokenizer),
            "bartpho_w": (MBartConfig, MBartForQuestionAnswering, PhobertTokenizer),
            "t5": (T5Config, T5ForQuestionAnswering, T5Tokenizer),
        ...
        if model_type == "auto":
            self.tokenizer = tokenizer_class.from_pretrained(model_name, use_fast=use_fast, **kwargs)
        else:
            self.tokenizer = tokenizer_class.from_pretrained(
                model_name, do_lower_case=self.args.do_lower_case, use_fast=use_fast, **kwargs
            )

            if model_type == 't5':
                special_tokens_dict = {
                    'additional_special_tokens': ['<bos>', '<cls>', '<s>'] + self.tokenizer.all_special_tokens}
